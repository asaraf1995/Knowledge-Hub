...
create table user(id int AUTO_INCREMENT primary key, name varchar(200),  email VARCHAR(50) NOT NULL unique)^;
...
create table user_activity(user_id int, FOREIGN KEY(user_id) references user(id), content_id int, FOREIGN KEY(content_id) references content_pages(content_id), page_id int, FOREIGN KEY(page_id) references content_pages(id), viewed_on date default(now()), primary key(user_id,content_id,page_id))^;
